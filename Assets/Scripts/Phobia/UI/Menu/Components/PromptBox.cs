...
namespace Phobia.ui.Menu.Components
{
	...
	public class PromptBox : MonoBehaviour
	{
		...
		[SerializeField] public Text messageText; // Make public for InitState access
		...
		[SerializeField] private TextAnchor messageAlignment = TextAnchor.MiddleCenter;
		...
		private void Awake()
		{
			// Ensure RectTransform exists before any access
			...
			if (promptSprite == null)
			{
				...
				try
				{
					...
					// Set up scaling to fit the PromptBox
					var parentSize = GetComponent<RectTransform>().rect.size;
					promptSprite.TargetSize = parentSize;
					promptSprite.ScaleToFit = true;
				}
				catch (System.Exception ex)
				{
					...
					try
					{
						...
						// Set up scaling for static texture too
						var parentSize = GetComponent<RectTransform>().rect.size;
						promptSprite.TargetSize = parentSize;
						promptSprite.ScaleToFit = true;
					}
			...
			// Ensure messageText exists and is a child of the PromptSprite
			if (messageText == null)
			{
				// Try to find an existing Text child
				var existingText = GetComponentInChildren<Text>(includeInactive: true);
				if (existingText != null && existingText.gameObject.name == "MessageText")
				{
				...
				else
				{
					...
					// Make text a child of the PromptSprite instead of the main PromptBox
					textObj.transform.SetParent(promptSprite.transform, false);

					var textRect = textObj.AddComponent<RectTransform>();
					// Set up text to fill the prompt sprite with some padding
				...
				messageText.font = messageFont != null ? messageFont : Resources.GetBuiltinResource<Font>("Arial.ttf");
				messageText.fontSize = messageFontSize;
				messageText.color = messageColor;
				messageText.alignment = messageAlignment;
				messageText.horizontalOverflow = HorizontalWrapMode.Wrap;
				messageText.verticalOverflow = VerticalWrapMode.Overflow;
				messageText.text = "";
				messageText.enabled = false;

				// Ensure text renders on top of the sprite
				messageText.transform.SetAsLastSibling();
			}
			else
			{
				// If messageText already exists but isn't a child of PromptSprite, reparent it
				if (promptSprite != null && messageText.transform.parent != promptSprite.transform)
				{
					messageText.transform.SetParent(promptSprite.transform, false);

					// Reset the rect transform to fit properly
		...
		{
			GameObject promptObj = new GameObject("PromptBox");
			// Ensure RectTransform is added immediately
			var rect = promptObj.AddComponent<RectTransform>();
			...
			if (promptBox.messageText == null)
			{
				...
				promptBox.messageText = textObj.AddComponent<Text>();
				promptBox.messageText.font = Resources.GetBuiltinResource<Font>("Arial.ttf");
				promptBox.messageText.fontSize = 24;
				promptBox.messageText.color = Color.white;
				promptBox.messageText.alignment = TextAnchor.MiddleCenter;
				promptBox.messageText.horizontalOverflow = HorizontalWrapMode.Wrap;
				promptBox.messageText.verticalOverflow = VerticalWrapMode.Overflow;
			}
			...
			// After creating the prompt box, set up the PhobiaSprite scaling
			if (promptBox.promptSprite != null)
			{
				promptBox.promptSprite.TargetSize = size;
				promptBox.promptSprite.ScaleToFit = true;
				promptBox.promptSprite.ApplyScaleToFit();
			}
